inString = '''pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
...
def isPassphraseValidExtra(phrase):
    if isPassphraseValid(phrase):
        words = phrase.split(' ')
        letters = [sorted(list(word)) for word in words]
        for i, word in enumerate(letters):
            for j, otherWord in enumerate(letters):
                if i != j:
                    if word == otherWord:
                        return False
...
for phrase in inString.split('\n'):
    if isPassphraseValidExtra(phrase):
        valids += 1
