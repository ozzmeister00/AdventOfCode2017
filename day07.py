...
class Program(object):
    def __init__(self, name, weight, namesAbove=None):
        self.name = name
        self.weight = weight
        self.discWeight = None
        self.namesAbove = namesAbove or []
        self.parent = ''

    def getTotalWeight(self, programs):
        if self.discWeight == None:
            self.discWeight = sum([programs[child].getTotalWeight(programs) for child in self.namesAbove]) + self.weight

        return self.discWeight


    def __str__(self):
        outStr = '{} ({})'.format(self.name, self.weight)
        if self.namesAbove:
            outStr += ' -> '
            outStr += ', '.join(self.namesAbove)
        return outStr
...
for line in puzzleInput.split('\n'):
    tokens = line.split(' ')
    name = tokens[0]
    weight = int(tokens[1].replace('(', '').replace(')', ''))
    aboves = []
    if len(tokens) > 2:
        aboves = [token.replace(',', '') for token in tokens[3:]]

    programs[name] = Program(name, weight, namesAbove=aboves)

childParent = {}
for program in programs:
    for child in programs[program].namesAbove:
        childParent[child] = programs[program].name
        programs[child].parent = program

topMosts = [program for program in programs if not programs[program].namesAbove]

def recurseDown(programName, childParent):
    #print(programName)
    if programName not in childParent:
        return programName

    programName = recurseDown(childParent[programName], childParent)
    return programName

for top in topMosts:
    bottom = recurseDown(top, childParent)

# recurse up all the weights
programs[bottom].getTotalWeight(programs)

# working our way up, find the tower that doesn't have the same weight as its siblings
def recurseUp(programName):
    children = programs[programName].namesAbove
    if children:
        weights = [programs[child].discWeight for child in children]
        # if all the children are balanced, keep looking up
        ...
        if len(set(weights)) == 1:
            ...
            oddOneOut = [k for k, v in Counter(weights).items() if v == 1][0]
            otherValue = list(set([k for k, v in Counter(weights).items() if v != 1]))[0]
            difference = oddOneOut - otherValue
            for child in children:
                if programs[child].discWeight == oddOneOut:
                    pass

def getMinority(weights):
    oddOneOut = None
    if len(list(set(weights))) > 1:
        oddWeight = [k for k, v in Counter(weights).items() if v == 1][0]
        oddOneOut = weights.index(oddWeight)

    return oddOneOut

def findUnbalanced(programName, default=None):
    childNames = programs[programName].namesAbove
    childWeights = [programs[childName].discWeight for childName in childNames]
    index = getMinority(childWeights)
    if index == None:
        return default

    suspect = childNames[index]
    unbalanced = findUnbalanced(suspect, default=suspect)
    return unbalanced

unbalanced = findUnbalanced(bottom)

# find its siblings
parent = programs[unbalanced].parent
children = programs[parent].namesAbove

expectedWeight = 0

for child in children:
    if programs[unbalanced].discWeight != programs[child].discWeight:
        expectedWeight = programs[child].discWeight

subWeight = programs[unbalanced].discWeight - programs[unbalanced].weight
correctWeight = expectedWeight - subWeight
print(correctWeight)
