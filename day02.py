inString = '''798	1976	1866	1862	559	1797	1129	747	85	1108	104	2000	248	131	87	95
...
def processInput(inString):
    """
    Takes the input string from day 2 and returns a list of ints for each row in the input string
    """
    rows = inString.split('\n')
    output = []
    for row in rows:
        output.append([int(i) for i in row.split('\t')])
    return output

def differenceMaxAndMin(row):
    return max(row) - min(row)

def findDivisibles(row):
    for i, startVal in enumerate(row):
        for j, testVal in enumerate(row):
            if i != j:
                if startVal % testVal == 0:
                    return startVal, testVal

def main():
    checksum = 0
    for row in processInput(inString):
        print(row)
        if row:
            #checksum += differenceMaxAndMin(row)
            x, y = findDivisibles(row)
