testInstructions = '''b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10'''

instructions = '''yxr inc 119 if nev != 6
piw dec -346 if tl != 4
...
commands = {'inc':increment,
            'dec':decrement
            }

def discoverRegisters(instructions):
    registers = {}
...
def processInstruction(instruction, registers):
    tokens = instruction.split(' ')
    target = tokens[0]
    command = commands[tokens[1]]
    value = int(tokens[2])
    #if = tokens[3]
    conditionA = registers[tokens[4]]
    conditional = conditions[tokens[5]]
    conditionB = int(tokens[6])

    print(target, command, value, conditionA, conditional, conditionB)

    if conditional(conditionA, conditionB):

        registers[target] = command(registers[target], value)

    return registers

#instructions = testInstructions

registers = discoverRegisters(instructions)
...
for instruction in instructions.split('\n'):
    ...
    currMax = max(registers.values())
    if currMax > maxValue:
        print(currMax, maxValue)
        maxValue = currMax
